...
public class HardwareStore {
     ...
     */
    public static String[][] getAllItemsFormatted() {
    	//sortItemList();
    	String[][] formattedArray = new String[itemList.size()][];
    	int counter = 0;
    	for(Item item : itemList) {
    		formattedArray[counter][0] = item.getIdNumber();
    		formattedArray[counter][1] = item.getName();
    		formattedArray[counter][2] = Integer.toString(item.getQuantity());
    		formattedArray[counter][3] = Float.toString(item.getPrice());
    		if(item instanceof Appliances) {
    			formattedArray[counter][4] = ((Appliances) item).getType();
    			formattedArray[counter][5] = ((Appliances) item).getBrand();
    		}
    		else if(item instanceof SmallHardwareItems) {
    			formattedArray[counter][4] = "Small Items";
    			formattedArray[counter][5] = ((SmallHardwareItems) item).getCategory();
    		}
    	counter++;
    	}
     ...
     */
    private static String getFormattedItemList(ArrayList<Item> items) {
    ...
    /**
   	 ...
   	 * @throws IOException 
   	 */
   	public void readItemsFile() throws IOException {
   		try 
   		{
   			...
   			while ((strLine = br.readLine()) != null) {
   				...
   				new SmallHardwareItems(ID, name, quantity, price, category);
   			}
    ...
    /**
   	 ...
   	 * @throws IOException 
   	 */
   	public static void readUsersFile() throws IOException {
   		try 
   		{
